...
class DraftListImagePreviewInspectorPanelViewController(RWidget):
    ...
    def _start_save_timer(self):
        self._save_debouncer.trigger_fn(self._save_config_and_notify)
    ...
    def _save_config_and_notify(self):
        ...
        deck_list_styles.is_main_deck_enabled = self._deck_list_image_generator_styles.is_main_deck_enabled
        ...
        self._configuration_manager.save_deck_list_image_generator_styles(
            deck_list_styles)
    ...
    def _main_deck_box_changed(self, val: bool):
        self._deck_list_image_generator_styles.is_main_deck_enabled = val
        self._start_save_timer()
    ...
    def _setup_view(self):
        self._layout_type_dropdown = RObjectComboBox([
            (DeckListImageGeneratorStyles.LayoutType.COST_CURVE.value,
             DeckListImageGeneratorStyles.LayoutType.COST_CURVE),
            (DeckListImageGeneratorStyles.LayoutType.GRID.value,
             DeckListImageGeneratorStyles.LayoutType.GRID)
        ])
        self._layout_type_dropdown.setCurrentText(
            self._deck_list_image_generator_styles.layout_type.value)
        self._layout_type_dropdown.currentIndexChanged.connect(
            self._layout_type_updated)

        vertical_layout = VerticalBoxLayout([
            HorizontalLabeledInputRow(
                "Layout type", self._layout_type_dropdown),
            ...
            HorizontalLabeledInputRow("Show main deck", RCheckBox(
                self._main_deck_box_changed, self._deck_list_image_generator_styles.is_main_deck_enabled)),
            ...
            HorizontalLabeledInputRow("Sort alphabetically", RCheckBox(
                self._alphabetical_box_changed, self._deck_list_image_generator_styles.is_sorted_alphabetically)),
            ...
            HorizontalLabeledInputRow("Full image preview (⚠️: This will increase image generation duration and memory usage)", RCheckBox(
                self._full_image_preview_changed, self._deck_list_image_generator_styles.is_full_image_preview)).set_word_wrap(True),

            HorizontalLabeledInputRow("Auto generate preview (⚠️: This will increase CPU usage)", RCheckBox(
                self._auto_generate_preview_changed, self._deck_list_image_generator_styles.is_auto_generate_preview))
        ]).add_spacer(RVerticallyExpandingSpacer())

        self.regenerate_button = PushButton(
        ...
        if self._configuration_manager.configuration.core_configuration.is_developer_mode:
            vertical_layout.add_widget(HorizontalLabeledInputRow("Visual debug", RCheckBox(
                self._visual_debug_box_changed, self._deck_list_image_generator_styles.is_visual_debug)),)
