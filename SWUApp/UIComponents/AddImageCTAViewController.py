...
class AddImageCTAViewController(RWidget, TransmissionReceiverProtocol):
    def __init__(self,
        ...
        background_url = urllib.parse.urljoin(
            "", self._asset_provider.image.sor_background.replace("\\", "/"))
    ...
    def _cta_clicked(self):
        ...
        prompt_generate_new_file_with_placeholder(
            self._asset_provider.image.swu_card_back)
    ...
    def _pressed_sound(self):
        try:
            ...
            self.sound_effect.setSource(QUrl.fromLocalFile(
                self._asset_provider.audio.r4_effect_path))
            print(
                f'playing sound effect: {self._asset_provider.audio.r4_effect_path}')
