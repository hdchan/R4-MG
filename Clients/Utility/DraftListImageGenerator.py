...
import sys
from ..Models import SWUTradingCardBackedLocalCardResource, ParsedDeckList
...
class DraftListImageGenerator:
    ...
    def generate_image(self, option: Option, 
                       parsed_deck_list: ParsedDeckList) -> Optional[Image.Image]:
        if option == DraftListImageGenerator.Option.COST_CURVE_LEADER_BASE_HORIZONTAL:
            return self.generate_cost_curve_with_horizontal_leader_base(parsed_deck_list)
        if option == DraftListImageGenerator.Option.COST_CURVE_LEADER_BASE_VERTICAL:
            return self.generate_cost_curve_with_vertical_leader_base(parsed_deck_list)
    ...
    def _create_leader_base_stacked_horizontal_image(self,
        ...
        image_paths = list(map(lambda x: x.asset_path, parsed_deck_list.first_leader_and_first_base))
    ...
    def _create_main_deck_cost_curve_image(self, 
        ...
        main_deck_image = stack_rows([
            create_card_type_row(lambda x: parsed_deck_list.all_units_with_cost(x), lambda x: parsed_deck_list.main_deck_with_cost(x)),
            create_card_type_row(lambda x: parsed_deck_list.all_upgrades_and_events_with_cost(x), lambda x: parsed_deck_list.main_deck_with_cost(x)),
        ])
    ...
    def generate_cost_curve_with_vertical_leader_base(self, parsed_deck_list: ParsedDeckList) -> Optional[Image.Image]:
        ...
        main_deck_image = self._create_main_deck_cost_curve_image(parsed_deck_list, 
                                                                  uniform_card_width, uniform_card_height)
        leader_base_image = self._create_leader_base_stacked_vertical_image(parsed_deck_list,
                                                                              uniform_card_height,
                                                                              uniform_card_width)

        leader_base_main_deck_image = self._create_transparent_image(main_deck_image.width + leader_base_image.width, 
        ...
        leader_base_main_deck_image.paste(main_deck_image, 
                           (leader_base_image.width, int(leader_base_main_deck_image.height / 2 - leader_base_main_deck_image.height/ 2)), 
                           main_deck_image)
        
        sideboard_image = self._create_sideboard_stacked_vertical_image(parsed_deck_list, uniform_card_width, uniform_card_height)
        sideboard_spacing = 20
    ...
    def generate_cost_curve_with_horizontal_leader_base(self, parsed_deck_list: ParsedDeckList) -> Optional[Image.Image]:
        ...
        result_image = self._create_transparent_image(max(main_deck_sideboard_image.width, leader_base_image.width), 
                                                      main_deck_sideboard_image.height + leader_base_image.height)
        ...
        result_image.paste(main_deck_sideboard_image, 
                           (int(result_image.width / 2 - main_deck_sideboard_image.width/ 2), leader_base_image.height), 
