...
class CustomDirectorySearchDataSource(DataSourceSelectedLocalCardResourceProtocol):
    ...
    def __init__(self,
        ...
        self._local_data_fetcher = DataFetcherLocal(DataFetcherLocal.Configuration(
            self._configuration_manager.configuration.network_delay_duration))
    ...
    def _directory_path(self) -> Optional[str]:
        # NOTE: we can't put trailing slash in configuration, otherwise it will append when reading
        return f'{self._configuration.custom_directory_search_path}/'

    def open_directory_path(self):
        if self._directory_path is not None:
            self._platform_service_provider.platform_service.open_file(
    ...
    def _retrieve_card_resource_for_card_selection(self, index: int, retry: bool = False):
        ...
        self._observation_tower.notify(
            LocalCardResourceSelectedFromDataSourceEvent(selected_resource, self))
        self._image_resource_processor_provider.image_resource_processor.async_store_local_resource(
            selected_resource, retry=retry)
        if self.delegate is not None:
            self.delegate.ds_did_retrieve_card_resource_for_card_selection(
    ...
    def search(self, search_configuration: SearchConfiguration):
        ...
        print(
            f'Custom directory search. card_name: {search_configuration.card_name}, search_configuration: {search_configuration}')
        ...
        def callback(result: Tuple[List[Tuple[str, str]], Optional[Exception]]):
            ...
            self._trading_card_providers = list(
                map(create_trading_card_resource, result_list))
            if self.delegate is not None:
                self.delegate.ds_completed_search_with_result(
                    self, search_configuration, error)
        ...
        self._local_data_fetcher.load(
            self._perform_search, callback, search_configuration=search_configuration)

    # MARK: - async work
    def _perform_search(self, args: Any) -> Tuple[List[Tuple[str, str]], Optional[Exception]]:
        search_configuration: SearchConfiguration = args.get(
            'search_configuration')

        search_filtered_string = "".join(
            filter(str.isalnum, search_configuration.card_name.lower()))

        def filter_the_result(card: Tuple[str, str]):
            # if search_configuration.card_name.replace(" ", "") == '':
            #     return True
            file_filtered_string = "".join(
                filter(str.isalnum, card[0].lower()))
        ...
        if self._directory_path is None:
            return [], None
        try:
            files = [f for f in os.listdir(self._directory_path) if os.path.isfile(
