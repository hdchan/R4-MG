...
class DataSourceImageResourceDeployerProvider(DataSourceImageResourceDeployerProviding, TransmissionReceiverProtocol):
    def __init__(self,
        ...
        self._current_websocket_ds: Optional[DataSourceImageResourceDeployerProtocol | WebSocketMessageReceiverProtocol] = None
    ...
    def handle_observation_tower_event(self, event: TransmissionProtocol) -> None:
        if type(event) is WebSocketStatusUpdatedEvent:
            if self._websocket_service.state == WebSocketServiceStatus.IS_CLIENT:
                self._current_websocket_ds = DataSourceImageResourceDeployerWebSocketClient(self._websocket_service, self._observation_tower)
