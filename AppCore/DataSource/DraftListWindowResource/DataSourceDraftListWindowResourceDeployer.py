...
from AppCore.Models import DraftListWindowConfiguration, LocalResourceDraftListWindow
...
class DataSourceDraftListWindowResourceDeployer(DataSourceDraftListWindowResourceDeployerProtocol, TransmissionReceiverProtocol):
    def __init__(self,
    ...
    def draft_list_windows(self) -> List[LocalResourceDraftListWindow]:
        return sorted(self._draft_list_windows, key=lambda x: x.created_date)

    def load_resources(self):
        start_event = DraftListWindowResourceLoadEvent(
            DraftListWindowResourceLoadEvent.EventType.STARTED)
        ...
        for file_name_with_ext in file_list[:]:
            ...
            if file_path.suffix == f'.{JSON_EXTENSION}':
                loaded_json = self._data_serializer.load(
                    f'{self._configuration.draft_list_windows_dir_path}/{file_name_with_ext}')
                if loaded_json is None:
                    continue
                window_config = DraftListWindowConfiguration.from_json(
                    loaded_json)
                draft_pack = self._data_source_draft_list.pack_for_draft_pack_identifier(
                    window_config.draft_pack_identifier)
        ...
        end_event = DraftListWindowResourceLoadEvent(
            DraftListWindowResourceLoadEvent.EventType.FINISHED)
    ...
    def delete_window_resource(self, resource: LocalResourceDraftListWindow):
        found: List[LocalResourceDraftListWindow] = list(
            filter(lambda x: x == resource, self._draft_list_windows))
        if len(found) > 0:
            resource = found[0]
    ...
    def _update_window_configuration(self, resource: LocalResourceDraftListWindow, new_configuration: DraftListWindowConfiguration):
        found_resources: List[LocalResourceDraftListWindow] = list(
            filter(lambda x: x == resource, self._draft_list_windows))
        ...
        if path_to_file.is_file():
            ...
            event = DraftListWindowResourceUpdatedEvent(
                copy.deepcopy(updated_resource), old_resource)
    ...
    def _generate_dir_if_needed(self):
        Path(self._configuration.draft_list_windows_dir_path).mkdir(
