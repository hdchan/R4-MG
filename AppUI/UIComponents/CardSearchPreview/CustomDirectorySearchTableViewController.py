...
class CustomDirectorySearchTableViewController(RWidget,
    ...
    def __init__(self,
        ...
        self._card_search_data_source = app_dependencies_provider.new_instance_custom_directory_search_data_source(
            self)
    ...
    def _load_source_labels(self, status_string: str = ""):
        self.search_source_label.setText(
            f'Search source: <a href="#open-directory">{self._card_search_data_source.source_display_name}</a> {status_string}')
    ...
    def handle_observation_tower_event(self, event: TransmissionProtocol):
        if type(event) is CardSearchEvent:
            if event.event_type == CardSearchEvent.EventType.STARTED:
                self._search_table_combo_view.set_search_components_enabled(
                    False)
            ...
            if event.seconds_since_predecessor is not None and event.source_type is CardSearchEvent.SourceType.LOCAL:
                print(
                    f"Custom search took :{event.seconds_since_predecessor}s")
