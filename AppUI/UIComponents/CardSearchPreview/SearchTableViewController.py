...
from AppCore.Observation.Events import CardSearchEvent, ConfigurationUpdatedEvent
...
class SearchTableViewController(RWidget,
    ...
    def __init__(self,
        ...
        search_source_label.setSizePolicy(
        ...
        app_dependencies_provider.shortcut_action_coordinator.bind_flip(
            self._flip_card, self)
        app_dependencies_provider.shortcut_action_coordinator.bind_focus_search(
            self._search_table_combo_view.set_search_focus, self)
        app_dependencies_provider.shortcut_action_coordinator.bind_reset_search(
            self._search_table_combo_view.reset_search, self)
        app_dependencies_provider.shortcut_action_coordinator.bind_search(
            self._search, self)
        app_dependencies_provider.shortcut_action_coordinator.bind_search_leader(
            self._search_secondary, self)
        app_dependencies_provider.shortcut_action_coordinator.bind_search_base(
            self._search_tertiary, self)
    ...
    def _load_source_labels(self, status_string: str = ""):
        ...
        if search_source_url is not None:
            ...
            self.search_source_label.setText(
                f'Search source: <a href="{search_source_url}">{self._card_search_data_source.source_display_name}</a> {status_string}')
            ...
            self.search_source_label.setText(
                f'Search source: <a href="{self.LinkKey.LOCALLY_MANAGED_DECKS}">{self._card_search_data_source.source_display_name}</a> {status_string}')
        else:
            self.search_source_label.setText(
                f'Search source: {self._card_search_data_source.source_display_name} {status_string}')
    ...
    def stc_did_select_history(self, stc: SearchTableComboViewController, index: int):
        ...
        result = self._card_search_data_source.get_search_configuration_from_history(
            index)
        if result is not None:
            search_configuration, _ = result
            self._search_table_combo_view.set_configuration(
    ...
    def stc_select_card_resource_for_card_selection(self, stc: SearchTableComboViewController, index: int) -> None:
        self._card_search_data_source.select_card_resource_for_card_selection(
            index)
