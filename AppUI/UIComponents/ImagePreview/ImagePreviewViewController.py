...
class ImagePreviewViewController(RWidget, TransmissionReceiverProtocol):
    def __init__(self,
        ...
        self._image_view.linkActivated.connect(
            self._handle_link_activated)  # should only connect once
    ...
    def _show_context_menu(self, pos: QPoint):
        ...
        def _notify_delegate_redownload_resource():
            if self._local_resource is not None:
                self._image_resource_processor.async_store_local_resource(
                    self._local_resource, retry=True)

        def _notify_delegate_rotate_right_image():
            if self._local_resource is not None:
                self._image_resource_processor.rotate_and_save_resource(
                    self._local_resource, -90)
        ...
        def open_file():
            if self._local_resource is not None:
                self._platform_service.open_file(
                    self._local_resource.image_path)

        def open_file_directory():
            if self._local_resource is not None:
                self._platform_service.open_file_directory_and_select_file(
                    self._local_resource.image_path)
        ...
        def copy_image_url():
            ...
            if cb is not None and self._local_resource is not None and self._local_resource.remote_image_url is not None:
                ...
                cb.setText(self._local_resource.remote_image_url,
        ...
        if self._local_resource is not None:
            ...
            if self._local_resource.is_ready:
                ...
                regenerate_preview.triggered.connect(
                    _notify_delegate_regenerate_preview)
                ...
                if self._local_resource.remote_image_url is not None:  # might need this even if resource is not ready
                    ...
                    redownload_resource.triggered.connect(
                        _notify_delegate_redownload_resource)
                ...
                reveal_action = QAction(
                    f"Reveal {action_label} in file explorer", self)
                ...
                if self._can_post_process:
                    ...
                    rotate_right_action.triggered.connect(
                        _notify_delegate_rotate_right_image)
                    ...
                    rotate_left_action.triggered.connect(
                        _notify_delegate_rotate_left_image)
        ...
        if self._local_resource.is_loading:
            ...
            if self._configuration.hide_image_preview:  # show text only
                self._image_view.setText(
                    self._dynamic_display_name(self._local_resource))
                ...
                if success:
                    ...
                    self._image_view.setText(
                        f'⚠️ No preview for {self._local_resource.display_name}. <a href="{self.LinkKey.REGENERATE_PREVIEW}">Regenerate</a>')

        else:  # failed to load
            self.loading_spinner.stop()
            if self._local_resource.remote_image_url is not None:
                self._image_view.setText(
                    f'⛔ {self._local_resource.display_name} not found. <a href="{self.LinkKey.REDOWNLOAD_IMAGE}">Redownload</a>')
            elif self._local_resource.can_be_replaced_with_placeholder:
                self._image_view.setText(
                    f'⛔ {self._local_resource.display_name} not found. <a href="{self.LinkKey.REGENERATE_PRODUCTION_FILE}">Generate placeholder</a>')
            else:
                self._image_view.setText(
                    f'⛔ {self._local_resource.display_name} not found')
        ...
        if self._local_resource is not None:
            if link == self.LinkKey.REDOWNLOAD_IMAGE:
                ...
                self._image_resource_processor.generate_placeholder(
                    self._local_resource, self._external_app_dependencies_provider.card_back_image_path)
                self._sync_image_view_state()

    def _toggle_resource_details_visibility(self):
        self._card_display_name.setHidden(
            self._configuration.hide_image_preview)
        self._image_info_widget.setHidden(
            not self._configuration.show_resource_details or self._local_resource is None)

    def _set_image_info(self):
        if self._local_resource is not None and self._local_resource.is_ready:
            self._card_display_name.setText(
                self._dynamic_display_name(self._local_resource))

            image_w, image_h = self._local_resource.size
            self._size_info_label.setText(f'Size: {image_w}W x {image_h}H')

            self._image_url_label.setHidden(
                self._local_resource.remote_image_url is None)
            if self._local_resource.remote_image_url is not None:
                url = self._local_resource.remote_image_url
                self._image_url_label.setText(
                    f'Source: <a href="{url}">{url}</a>')

            self._created_date_label.setText(
                f'Created: {self._local_resource.created_date.strftime("%m/%d/%Y, %I:%M %p")}')
    ...
    def handle_observation_tower_event(self, event: TransmissionProtocol):
        if (type(event) is ConfigurationUpdatedEvent or
            type(event) is DataSourceImageResourceDeployerStateUpdatedEvent or
        ...
        if type(event) is LocalCardResourceFetchEvent:
            if self._local_resource is not None and self._local_resource.image_preview_path == event.local_resource.image_preview_path:
                ...
                if event.event_type == LocalCardResourceFetchEvent.EventType.FAILED:
                    print(
                        f"Failed resource: {self._local_resource.image_preview_path}")
                elif event.event_type == LocalCardResourceFetchEvent.EventType.FINISHED:
                    print(
                        f"Reloading resource: {self._local_resource.image_preview_path}")
