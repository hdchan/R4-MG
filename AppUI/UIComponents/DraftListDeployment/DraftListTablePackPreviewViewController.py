...
from PyQt5.QtWidgets import (QFrame, QScrollArea, QSizePolicy,
                             QSpacerItem, QVBoxLayout, QWidget, QLabel)
...
from .DraftListLineItemHeaderViewController import DraftListLineItemHeaderViewController
...
class DraftListTablePackPreviewViewController(QWidget, TransmissionReceiverProtocol, DraftListLineItemViewControllerDelegate):
    ...
    def _setup_view(self):
        ...
        self._scroll_view.setWidget(cells_container_widget)
    ...
    def _sync_list(self):
        ...
        self.clear_list()
        if self._delegate is not None and self._delegate.dlp_pack_index is not None:
            pack_index = self._delegate.dlp_pack_index
            aggregate_list: bool = not self._delegate.dlp_is_staging_view and self._stylesheet.is_list_aggregated
            
            list_to_enumerate = self._data_source_draft_list.resource_list_for_pack(pack_index)
            externally_modified_list = self._external_app_dependencies_provider.draft_resource_list(self._data_source_draft_list.resource_list_for_pack(pack_index), aggregate_list)
            if externally_modified_list is not None:
                list_to_enumerate = externally_modified_list # use external implementation if present
            
            pack_name = self._data_source_draft_list.pack_name(pack_index)
            if pack_name:
                self._cells_container_layout.addWidget(DraftListLineItemHeaderViewController(self._app_dependencies_provider, self._stylesheet, pack_name))
            ...
            for (card_index, local_resource) in enumerate(list_to_enumerate):
                ...
                line_item = DraftListLineItemViewController(self._app_dependencies_provider,
                                                            ...
                                                            pack_index, 
                                                            card_index, 
                                                            self._delegate.dlp_is_staging_view)
                line_item.set_delegate(self)
                self._cells_container_layout.addWidget(line_item)
            self._cells_container_layout.addSpacerItem(QSpacerItem(0, 0, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Expanding))
    ...
    def _sync_styles(self):
        ...
        self._cells_container_layout.setSpacing(stylesheet.cell_spacing)
    ...
    def handle_observation_tower_event(self, event: TransmissionProtocol) -> None:
        if type(event) == DraftListUpdatedEvent or \
